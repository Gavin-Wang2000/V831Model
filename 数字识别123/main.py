...
import time
input_size = (224, 224)
model = "model-40561.awnn.mud"
labels = ['1', '3', '2']
anchors = [5.16, 2.97, 4.31, 5.06, 3.81, 4.8, 5.94, 4.64, 3.36, 4.47]
...
class YOLOv2:
    ...
    def draw(self, img, boxes, probs):
        for i, box in enumerate(boxes):
            ...
            return num_msg
def main():
    ser = serial.Serial("/dev/ttyS1", 115200, timeout=1000)
    ...
    while True:
        ...
        num_result = yolov2.draw(img, boxes, probs)
        if num_result != None:
            ser.write((f"\r{num_result}\n").encode("gbk"))
        display.show(img)
